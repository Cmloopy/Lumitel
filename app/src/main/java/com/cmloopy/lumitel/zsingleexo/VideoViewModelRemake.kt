...
class VideoViewModelRemake: ViewModel() {
    ...
    fun getInfoChannel(msisdn: String){
        viewModelScope.launch {
            try {
                val result = channelRepository.getInfoMyChannel(msisdn)
    ...
    private fun loadVideoHot(idVideo: Int, msisdn: String) {
        viewModelScope.launch {
            try {
                ...
                val firstVideo = videoRepository.getInfoVideo(idVideo = idVideo, msisdn)
                finalList.add(firstVideo)
                val result = videoRepository.getVideoHot(msisdn)
    ...
    private fun loadVideos(idVideo: Int, idCate: Int, msisdn: String) {
        viewModelScope.launch {
            try {
                ...
                val firstVideo = videoRepository.getInfoVideo(idVideo = idVideo, msisdn)
                finalList.add(firstVideo)
                val result = videoRepository.getVideoByCategory(idCategory = idCate, msisdn)
                val safeList = result.map { it.copy(aspecRatio = it.aspecRatio ?: "1.5") }
                safeList.forEach {
                    finalList.add(it)
                }
    ...
    private fun loadVideoFromChannel(idVideo: Int, idChannel: Int, isShort: Boolean, msisdn: String){
        viewModelScope.launch {
            try {
                ...
                finalList.add(firstVideo)
                if (!isShort) {
                    val result = videoRepository.getVideoByChannel(channelId =  idChannel, msisdn)
                    val safeList = result.map { it.copy(aspecRatio = it.aspecRatio ?: "1.5") }
                    safeList.forEach {
                        finalList.add(it)
                    }
                    ...
                    val result = videoRepository.getShortByChannel(channelId =  idChannel, msisdn)
                    val safeList = result.map { it.copy(aspecRatio = it.aspecRatio ?: "1.5") }
                    safeList.forEach {
                        finalList.add(it)
                    }
